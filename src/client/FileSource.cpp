...
std::string
ExtLower(const std::string& fileName)
{
...
bool
BuildFileMetadata(const std::string& localPath, const std::string& id,
	FileMetadata& out)
{
	struct stat st;
	...
	out.modified = ToISO8601(st.st_mtime);
	out.accessed = ToISO8601(st.st_atime);
