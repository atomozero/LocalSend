...
void
WriteResponse(int fd, const HttpServerResponse& r)
{
	char head[512];
...
SocketHttpServer::Run()
{
	...
	while (fRunning) {
		...
		if (client < 0) {
			if (!fRunning)
				break; // Stop() ha chiuso il socket: uscita pulita
			continue;
		}
...
SocketHttpServer::HandleConnection(int clientFd)
{
	...
	// streaming su disco e' un raffinamento, vedi docs/02-design-L1.md.)
