...
HttpResponse readResponse(int fd) {
    ...
    while (pos < head.size()) {
        ...
        if (colon != std::string::npos) {
            ...
            if (key == "transfer-encoding" && toLower(val).find("chunked") != std::string::npos)
                chunked = true;
        }
    }
    ...
    if (contentLength >= 0) {
        ...
        while ((n = recv(fd, tmp, sizeof(tmp), 0)) > 0)
            body.append(tmp, static_cast<size_t>(n));
        (void)chunked; // decodifica chunked non necessaria per i body brevi di L0
    }
...
std::string buildHead(const std::string& host, int port, const std::string& path,
...
HttpResponse SocketHttpClient::post(const std::string& host, int port,
...
HttpResponse SocketHttpClient::postFile(const std::string& host, int port,
                                        const std::string& path,
                                        const std::string& contentType,
    ...
    while ((n = fread(buf, 1, sizeof(buf), f)) > 0) {
        if (!sendAll(fd, buf, n)) { ok = false; break; }
    }
    fclose(f);
